...
void kill_alarm(int sig_num)
{
	printf("\n\nSo far, '%d' Ctrl-C presses were counted\n\n", ctrl_c_count);
	printf("\nToo much time has passed. Exiting now.\n", NULL);
	fflush(stdout);
	exit(0);
}
...
int main(int argc, char* argv[])
{
  ...
  /* setup mask_set */
	sigfillset(&mask_set);
	stopsig.sa_mask=mask_set;
	alarmsig.sa_mask=mask_set;
	sigdelset(&mask_set, SIGALRM);
	interruptsig.sa_mask=mask_set;

  /* set signal handlers */
	interruptsig.sa_handler=catch_int;
	stopsig.sa_handler=catch_tstp;
	alarmsig.sa_handler=kill_alarm;
